...
import { encodeFunctionData, getAddress, zeroAddress, maxUint128, parseAbi, publicActions, } from "viem";
...
export const removeLiquidityV3 = async (account, tokenId, percent) => {
    const calldatas = [];
    ...
    const liquidity = new Percent(percent, 100).multiply(positionInfo[7]).quotient;
    // remove liquidity
    ...
    if (involvesETH) {
        ...
        const token0Amount = PositionMath.getToken0Amount(slot0[1], positionInfo[5], positionInfo[6], slot0[0], positionInfo[7]);
        const discountedAmount0 = new Percent(percent, 100).multiply(token0Amount).quotient;
        const token1Amount = PositionMath.getToken1Amount(slot0[1], positionInfo[5], positionInfo[6], slot0[0], positionInfo[7]);
