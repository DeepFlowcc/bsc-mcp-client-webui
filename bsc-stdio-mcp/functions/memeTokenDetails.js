export const memeTokenDetail = async (tokenName) => {
    ...
    if (!bnbPriceRes.ok)
        throw new Error(`Failed to fetch BNB price: ${bnbPriceRes.status}`);
    if (!tokenRes.ok)
        throw new Error(`Failed to fetch token details: ${tokenRes.status}`);
