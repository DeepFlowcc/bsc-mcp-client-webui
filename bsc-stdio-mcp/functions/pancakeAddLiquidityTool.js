...
import { Pool, Position, nearestUsableTick, encodeSqrtRatioX96 } from '@pancakeswap/v3-sdk';
import { ChainId, CurrencyAmount, Percent } from '@pancakeswap/sdk';
...
async function approveTokensIfNeeded(account, token, spender, amount) {
    if (!token.isNative) {
...
function sortTokens(tokenA, tokenB, amountA, amountB) {
 ...
 */
export async function addLiquidityV3(tokenA, tokenB, fee, amountA, amountB, account, slippageTolerance = new Percent('50', '10000'), // default 0.5%
deadline = Math.floor(Date.now() / 1000) + 20 * 60, // default 20 minutes
priceLower = 0.8, priceUpper = 1.2) {
    await Promise.all([
    ...
    const pool = new Pool(token0, token1, fee, slot0[0].toString(), // sqrtPriceX96
