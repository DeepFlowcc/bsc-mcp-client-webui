...
export const myPosition = async (accountAddress) => {
    ...
    const nftIds = await publicClient.multicall({
        contracts: nftCalls,
        allowFailure: false,
    });
    ...
    const poolTokenInfos = await Promise.all(positions.map(async (position) => {
        const tokenInfos = await Promise.all([
            getTokenInfo(position[2]),
            getTokenInfo(position[3]),
        ]);
        return {
            token0: tokenInfos[0],
            token1: tokenInfos[1],
    ...
    const slot0s = await publicClient.multicall({
