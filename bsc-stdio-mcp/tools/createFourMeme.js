...
export function registerCreateMemeToken(server) {
    server.tool("Create_FourMeme_Token", "🎭Launch a new meme token on four.meme platform", CreateFourMemeSchema.shape, async (args_) => {
        try {
            ...
            const logData = transaction.logs.find((log) => log.topics.length != 0 &&
